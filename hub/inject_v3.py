#!/usr/bin/env python3
"""
Injects HTML_V3, /v3 route, and _serve_v3 into dashboard.py.
Run on the board: python3 inject_v3.py
Safe — only adds, never modifies existing code.
"""

DASHBOARD = '/home/arduino/ArduinoApps/juice_battle/hub/dashboard.py'

with open(DASHBOARD, 'r') as f:
    content = f.read()

# ── Safety checks ──────────────────────────────────────────────────────────
assert 'HTML_V2 = """' in content,      "ERROR: HTML_V2 not found"
assert "'/v2', 'index_v2'" in content,  "ERROR: v2 route not found"
assert 'def _serve_v2' in content,      "ERROR: _serve_v2 not found"
assert 'HTML_V3' not in content,        "ERROR: HTML_V3 already exists — aborting"
...
/* CENTRE COLUMN */
.card-centre { background: rgba(8,8,8,0.82); border: 1px solid #1a1a1a; border-radius: 14px; padding: 10px; display: flex; flex-direction: column; gap: 6px; backdrop-filter: blur(2px); }
...
# ── Insertion 1: HTML_V3 after HTML_V2 block (after line 1477, i.e. after '</html>"""') ──
V2_END_MARKER = '</html>"""\n\n\nclass Dashboard:'
V3_AFTER_MARKER = '</html>"""\n'

# Find the exact split point — after HTML_V2's closing tag, before class Dashboard
split_marker = '\n\n\nclass Dashboard:'
idx = content.find(split_marker)
assert idx > 0, "ERROR: Could not find 'class Dashboard:' insertion point"

before_class = content[:idx]
class_onwards = content[idx:]

# ── Insertion 2: /v3 route — after the /v2 add_url_rule line ──
V2_ROUTE = "self._app.add_url_rule('/v2', 'index_v2', self._serve_v2)"
V3_ROUTE = "\n        self._app.add_url_rule('/v3', 'index_v3', self._serve_v3)"
class_onwards = class_onwards.replace(V2_ROUTE, V2_ROUTE + V3_ROUTE, 1)

# ── Insertion 3: _serve_v3 method — after _serve_v2 method ──
SERVE_V2_BLOCK = '''    def _serve_v2(self):
        """Serve the crowd-facing v2 dashboard."""
        return render_template_string(HTML_V2)'''

SERVE_V3_BLOCK = '''    def _serve_v3(self):
        """Serve the crowd-facing v3 dashboard with atmospheric background and cause panel."""
        return render_template_string(HTML_V3)'''

class_onwards = class_onwards.replace(SERVE_V2_BLOCK, SERVE_V2_BLOCK + '\n\n' + SERVE_V3_BLOCK, 1)

# ── Write output ──────────────────────────────────────────────────────────
new_content = before_class + HTML_V3 + class_onwards
...
checks = [
    ('HTML_V2 preserved',   'HTML_V2 = """' in v),
    ('HTML_V3 added',       'HTML_V3 = """' in v),
    ('/v2 route',           "'/v2', 'index_v2'" in v),
    ('/v3 route',           "'/v3', 'index_v3'" in v),
    ('_serve_v2',           'def _serve_v2' in v),
    ('_serve_v3',           'def _serve_v3' in v),
    ('bg-canvas',           'bg-canvas' in v),
    ('marketing-zone',      'marketing-zone' in v),
    ('welcome-band',        'welcome-band' in v),
    ('Natural Earth map',   '199.5,63.1' in v),
    ('socket.io preserved', 'socket.on' in v),
]
print()
...
print()
print('SUCCESS — restart the service to see /v3' if all_ok else 'SOME CHECKS FAILED — restore from backup')
